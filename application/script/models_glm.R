...
mrbs.lm <- function(k_can, 
    ...
    if(substr(criterion_setting,1,2)=="cv"){
        ...
        cv_result <- cv.glm_self_glmfit(phi_for_cv = phi_for_cv, Y_for_cv = Y_for_cv, k_can = k_can, k = 5, log_normal = log_normal)
        result <- c(cv_ent = cv_result$cv_ent$mean)
        
    }else{
        phi_train = subset_phi(phi_full, M_up, k_can, p_var)
        model = fastglm::fastglm(x = phi_train, y = Y_train)
        
        result <- c(aic = aic.bic(model)["aic"], # aic.bic_self(phi_train, Y_train, model$coefficients)['aic']
                    bic = aic.bic(model)["bic"]) # aic.bic_self(phi_train, Y_train, model$coefficients)['bic']   
...
mrbs.select.one <- function(k_can, 
    ...
    if(substr(criterion_setting,1,2)=="cv"){
        ...
        colnames(criterion_result_df) = c("cv_ent")
    }else{
...
mrbs.multi.svc <- function(formula,
    ...
    model <- glm(Y_train ~ phi_train-1, family = "binomial")
